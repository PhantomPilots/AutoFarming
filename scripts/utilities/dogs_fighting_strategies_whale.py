...
class DogsBattleStrategy(IBattleStrategy):
    ...
    def _floor_2_phase_2(self, hand_of_cards: list[Card], picked_cards: list[Card]) -> int:
        ...
        available_ids = [i for i, card in enumerate(hand_of_cards) if card.card_type != CardTypes.DISABLED and
                         not (find(vio.ghel_aoe1, card.card_image) or find(vio.ghel_aoe2, card.card_image) or 
                              find(vio.ghel_ult, card.card_image) or find(vio.milim_aoe, card.card_image) or 
                              find(vio.milim_ult, card.card_image))]
    ...
    def _floor_2_phase_3(self, hand_of_cards: list[Card], picked_cards: list[Card]) -> int:
        ...
        print(f"Floor 2, Phase 3: Milim frozen: {has_unv_milim}, Ghel frozen: {has_unv_ghel}, Thor frozen: {has_unv_thor}, Merlin frozen: {has_unv_lolimerl}")
        ...
        if is_frozen and (has_unv_milim or has_unv_ghel or has_unv_thor or has_unv_lolimerl):
            ...
            if card_turn == 0:
                ...
                if idx != -1 and self._find_card(hand_of_cards, vio.ghel_aoe2) != -1:
            ...
            elif card_turn == 1 and any(find(vio.ghel_aoe1, card.card_image) for card in picked_cards):
                idx = self._find_card(hand_of_cards, vio.ghel_aoe2)
                if idx != -1:
    ...
    def _is_st_or_lolimerl_card(self, card: Card) -> bool:
        ...
        return (find(vio.lolimerl_st, card.card_image) or find(vio.lolimerl_aoe, card.card_image) or 
                ...
                find(vio.thor_1, card.card_image) or find(vio.thor_2, card.card_image))
    ...
    def _find_aoe_card(self, hand_of_cards: list[Card], unit=None) -> int:
        ...
        aoe_cards = [vio.ghel_aoe1, vio.ghel_aoe2, vio.lolimerl_aoe, vio.milim_aoe, 
                     ...
                     vio.thor_ult, vio.ghel_ult, vio.milim_ult, vio.lolimerl_ult]
        if unit == "ghel":
            aoe_cards = [vio.ghel_aoe1, vio.ghel_aoe2, vio.unv_ghel_aoe1, vio.unv_ghel_aoe2, vio.ghel_ult]
