...
from utilities.utilities import capture_window, click_im, determine_card_merge, find
...
class DogsFloor4WhaleBattleStrategy(IBattleStrategy):
        ...
        if meli_ids and esca_ids:
            if not played_esca:
                idx = self._best_card_from_priority(
                    hand_of_cards, [("escalin_st",), ("escalin_aoe",), ("escalin_ult",)]
                )
                if idx != -1:
                    return idx
    ...
    def _phase2_odd_turn_whale_action(
        ...
        if played_hi_stun or played_meli_ult or sins_played_count >= 3:
            ...
            if nuke_action != -1:
                return nuke_action
            filler_action = self._phase2_support_filler_action(
                hand_of_cards, picked_cards, allow_silver_stun_merge=False
            )
            if filler_action != -1:
                return filler_action
            fallback_action = self._phase2_last_resort_action(hand_of_cards, picked_cards)
    ...
    def _card_template_name(self, card: Card, template_names: Sequence[str]) -> str | None:
        return next(
            (template_name for template_name in template_names if self._card_matches_any(card, (template_name,))), None
        )
